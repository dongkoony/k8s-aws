...
# 승인 워크플로우 데코레이터
def approval_required(action_name, impact_message):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            user = kwargs.get("user", "unknown")
            send_approval_request(user, action_name, impact_message)
            send_action_log(user, action_name, "대기(관리자 승인)")
...
# 민감 작업 래핑 (승인 워크플로우 적용)
@approval_required("EC2 인스턴스 중지", "이 작업은 모든 EC2 인스턴스가 중지되어 서비스가 중단될 수 있습니다.")
def stop_ec2_instance(*args, **kwargs):
    result = real_stop_ec2_instance(*args, **kwargs)
    user = kwargs.get("user", "unknown")
    send_action_log(user, "EC2 인스턴스 중지", "실행됨")
    send_result_notification("EC2 인스턴스 중지", "관리자")
    return result

@approval_required("파드 삭제", "이 작업은 파드가 삭제되어 서비스에 영향이 있을 수 있습니다.")
...
@approval_required("디플로이먼트 삭제", "이 작업은 디플로이먼트가 삭제되어 서비스에 영향이 있을 수 있습니다.")
